...
const Cart = () => {
    ...
    if (cartItems.length === 0) return (
        <div style={{ padding: `60px ${PAD}`, textAlign: 'center', background: '#FFFBF8', minHeight: '60vh' }}>
            ...
            <h2 style={{ fontSize: 15, fontWeight: 900, color: '#1C1C1E', marginBottom: 10 }}>Your bag is empty</h2>
            <p style={{ fontSize: 15, color: '#6E6E73', marginBottom: 36 }}>Looks like you haven't added anything yet.</p>
    ...
    return (
        <div style={{ background: '#FFFBF8', minHeight: '80vh', padding: `36px ${PAD} 80px` }}>
            ...
            {/* Page title */}
            <h1 style={{ fontSize: 'clamp(20px,2.4vw,30px)', fontWeight: 900, color: '#1C1C1E', marginBottom: 32, letterSpacing: '-.02em' }}>
                My Cart
                <span style={{ fontSize: 15, fontWeight: 600, color: '#9CA3AF', marginLeft: 12 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 28, alignItems: 'start' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                        ...
                        return (
                            <div key={item._id} style={{
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    <div style={{ fontSize: 15, fontWeight: 800, color: '#FC2779', textTransform: 'uppercase', letterSpacing: '.1em', marginBottom: 6 }}>
                                        {item.brand}
                                    </div>
                                    <Link to={`/product/${item._id}`} style={{ textDecoration: 'none' }}>
                                        <div style={{ fontSize: 15, fontWeight: 700, color: '#1C1C1E', lineHeight: 1.4, marginBottom: 10 }}>
                                            {item.name}
                                        </div>
                                    </Link>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 16 }}>
                                        <span style={{ fontSize: 15, fontWeight: 900, color: '#1C1C1E' }}>₹{ep.toLocaleString('en-IN')}</span>
                                        {hasDisc && (
                                            <>
                                                <span style={{ fontSize: 13, color: '#B0B0B0', textDecoration: 'line-through' }}>₹{item.price.toLocaleString('en-IN')}</span>
                                                <span style={{ fontSize: 12, fontWeight: 800, color: '#16A34A' }}>{discPct}% off</span>
                                            </>
                                        )}
                                    </div>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
                                        <div style={{
                                            ...
                                            <span style={{ padding: '0 18px', fontSize: 12, fontWeight: 800, color: '#1C1C1E', userSelect: 'none', minWidth: 20, textAlign: 'center' }}>
                                                {item.qty}
                                            </span>
                                        ...
                                        <span style={{ fontSize: 15, fontWeight: 700, color: '#6E6E73' }}>
                                ...
                                <button
                                    ...
                                    style={{
                                        ...
                                        width: 38, height: 38,
                                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                                        cursor: 'pointer', color: '#FC2779', fontSize: 13,
                                        transition: 'all .2s',
                                    }}
                    ...
                    <Link to="/products" style={{
                        ...
                        fontSize: 13, fontWeight: 700, color: '#FC2779',
                ...
                <div style={{ position: 'sticky', top: 100 }}>
                    <div style={{
                        ...
                        <h2 style={{ fontSize: 15, fontWeight: 900, color: '#1C1C1E', marginBottom: 24, paddingBottom: 16, borderBottom: '1.5px solid #F0E0EA', letterSpacing: '-.01em' }}>
                            Price Details
                        </h2>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 20 }}>
                            {[
                                ...
                                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <span style={{ fontSize: 15, color: '#555', fontWeight: 500 }}>{row.label}</span>
                            ...
                            <div style={{
                                ...
                                <span style={{ fontSize: 15, fontWeight: 900, color: '#1C1C1E' }}>Total Amount</span>
                                <span style={{ fontSize: 15, fontWeight: 900, color: '#1C1C1E' }}>₹{finalTotal.toLocaleString('en-IN')}</span>
                            </div>
                        </div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginTop: 14, color: '#9CA3AF', fontSize: 12, fontWeight: 500 }}>
                    ...
                    <div style={{
                        ...
                        <h3 style={{ fontSize: 15, fontWeight: 800, color: '#1C1C1E', marginBottom: 14 }}>Available Offers</h3>
                        {[
                            ...
                            <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', marginBottom: i < 2 ? 12 : 0 }}>
                                <span style={{ fontSize: 13, flexShrink: 0, marginTop: 1 }}>{offer.icon}</span>
                                <span style={{ fontSize: 12, color: '#555', lineHeight: 1.5 }}>{offer.text}</span>
                            </div>
                        ))}
                    </div>
                </div>
